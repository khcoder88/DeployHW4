...
async function login(req, res) {
    ...
    try {
        // get user's password from DB
        let actualPassword = await db.get('SELECT password FROM users WHERE username = ?', [username]);
...
async function createAccount(req, res) {
    ...
    try {
        ...
        if (!actualPassword) {
            ...
            let response = await db.run("INSERT INTO users(username, password) VALUES(?, ?)", [username, hashedPassword]);
...
app.get("/api/book", async (req, res) => {
    ...
    let bookId;
    let genre;
    bookId = req.query.id ? parseInt(req.query.id, 10) : undefined;
...
app.post("/api/book", async (req, res) => {
    ...
    try {
        ...
        result = await db.all("INSERT INTO books(author_id, title, pub_year, genre, username) VALUES(?, ?, ?, ?, ?) RETURNING id", [author_id, title, pub_year, genre, username]);
...
app.post("/api/author", async (req, res) => {
    ...
    try {
        result = await db.all("INSERT INTO authors(name, bio) VALUES(?, ?) RETURNING id", [name, bio]);
    }
    catch (err) {
        let error = err;
        return res.status(500).json({ error: error.toString() });
...
app.delete("/api/book", async (req, res) => {
    ...
    if (authorId !== undefined) {
        ...
        //console.log("running....");
        result = await db.run("DELETE FROM books WHERE id = ?", [req.query.id]);
        if (result.changes === 1) {
...
app.delete("/api/author", async (req, res) => {
    ...
    if (authorId !== undefined) {
        ...
        result = await db.run("DELETE FROM books WHERE author_id = ?", [req.query.id]);
        result = await db.run("DELETE FROM authors WHERE id = ?", [req.query.id]);
        if (result.changes === 1) {
            return res.status(204).json({ id: result });
        }
        else {
...
app.put("/api/book", async (req, res) => {
    ...
    const bookId = req.query.id ? parseInt(req.query.id, 10) : undefined;
    ...
    const queryStr = `UPDATE books SET ${queryArr.join(", ")} WHERE id = ? RETURNING id`;
    let dbResult;
    try {
        //console.log("try block");
        if (bookId) {
            dbResult = await db.get(queryStr, [...queryValues, bookId]);
        }
